...
wp.customize.controlConstructor['buttonset'] = wp.customize.Control.extend({
  ...
  jQuery(document).ready(function ($) {
    ...
    $(".alpha-color-control").each(function () {
      ...
      colorPickerOptions = {
        ...
        palettes: newslistColorPalette.colorPalettes // Use the passed in palette.
...
jQuery(window).load(function () {
  ...
  jQuery('.customize-control-radio-image .buttonset').buttonset(); // Handles setting the new value in the customizer.

  jQuery('.customize-control-radio-image input:radio').change(function () {
...
wp.customize.controlConstructor["range"] = wp.customize.Control.extend({
  ready: function ready() {
    ...
    var control = this,
        ...
        input_default,
        changeAction,
        newslist_range_input_number_timeout; // Update the text value
    ...
    function autocorrect_range_input_number(input_number, timeout) {
      ...
      clearTimeout(newslist_range_input_number_timeout);
      newslist_range_input_number_timeout = setTimeout(function () {
        if (isNaN(value)) {
...
jQuery(function ($) {
  $(document).on('click', '.customizer-reset', function (event) {
...
/***/ "./classes/customizer/custom-control/section-order/assets/section-order.js":
/*!*********************************************************************************!*\
  !*** ./classes/customizer/custom-control/section-order/assets/section-order.js ***!
  \*********************************************************************************/
/*! no static exports found */
...
wp.customize.controlConstructor["newslist-section-order"] = wp.customize.Control.extend({
  ready: function ready() {
    "use strict";

    var control = this;
    jQuery("#customize-control-" + control.params.id).find(".newslist-sortable-section").sortable({
      stop: function stop(event, ui) {
        var controlid = control.params.id;
        var finalOrder = [];
        var orderli = jQuery(event.target).find("li");
        orderli.each(function (i, v) {
          var val = jQuery(this).data("val");
          finalOrder.push(val);
        });
        control.setting.set(JSON.stringify(finalOrder));
      }
    }).disableSelection();
  }
});
...
__webpack_require__(/*! ../../classes/customizer/custom-control/section-order/assets/section-order.js */ "./classes/customizer/custom-control/section-order/assets/section-order.js");
