...
if ( !class_exists( 'Newslist_Customizer' ) ) {
	class Newslist_Customizer extends Newslist_Helper {
		/**
		 * The object instance.
		 *
		 * @static
		...
		public static function add_custom_control( $control ) {
			if ( !is_array( $control ) ) {
				$err = esc_html__( 'Invalid argument passed for Custom Control. Must be an array with "type" & "class" key.', 'newslist' );
				self::add_error( $err );
				return;
			}
			...
			if ( !isset( $control['type'] ) ) {
				$err = esc_html__( 'Custom Control must contain "type" key ( type of control ).', 'newslist' );
				self::add_error( $err );
				$flag = false;
			}

			if ( !isset( $control['class'] ) ) {
				$err = esc_html__( 'Custom Control must contain "class" key ( name of custom class ).', 'newslist' );
				self::add_error( $err );
				$flag = false;
			}

			if ( !isset( $control['sanitize'] ) ) {
				$err = esc_html__( 'Custom Control must contain "sanitize" key ( a callback function ).', 'newslist' );
				self::add_error( $err );
				$flag = false;
			}
		...
		public static function add_custom_section( $section ) {
			if ( !is_array( $section ) ) {
				$err = esc_html__( 'Invalid argument passed for Custom Section. Must be an array with "type" & "class" key.', 'newslist' );
				self::add_error($err);
				return;
			}
			...
			if ( !isset( $section['type'] ) ) {
				$err = esc_html__( 'Custom Section must contain "type" key ( type of section ).', 'newslist' );
				self::add_error($err);
				$flag = false;
			}

			if ( !isset( $section['class'] ) ) {
				$err = esc_html__( 'Custom Section must contain "class" key ( name of section class ).', 'newslist' );
				self::add_error( $err );
				$flag = false;
			}
				...
				foreach ( $args['fields'] as $field ) {
					...
					if ( isset( $field['id'] ) ) {
						...
						if (array_key_exists($field_id, self::$settings)) {
							...
							$msg = sprintf( esc_html__( 'Field with ID: %s already exists.', 'newslist' ), $field['id'] );
							self::add_error( $msg );
						}
		...
		private static function validate_argument( $args ) {
			...
			if ( !is_array( $args ) ) {
				...
				$msg = esc_html__( 'You must pass array as an argument in set method.', 'newslist' );
			...
			if ( !isset($args['section'] ) ) {
				...
				$msg = esc_html__( 'No section found.', 'newslist' );
				self::add_error( $msg, 2 );
			...
			if ( !is_array( $args['section'] ) ) {
				...
				$msg = sprintf(
					...
					esc_html__( 'Invalid argument', 'newslist' ),
					esc_html__( 'section', 'newslist' ),
					esc_html__( 'Must be a an array with atleast an id, title...', 'newslist' )
				);

				self::add_error( $msg, 2 );
			} elseif ( !isset( $args['section']['id'] ) ) {

				$msg = esc_html__( 'Section id is missing.', 'newslist' );
				self::add_error( $msg, 2 );
			}
			...
			if ( isset( $args['fields'] ) && !is_array($args['fields'] ) ) {
				...
				$msg = sprintf(
					'%s <strong><em>%s</em></strong>. %s',
					esc_html__( 'Invalid argument', 'newslist' ),
					esc_html__( 'fields', 'newslist' ),
					esc_html__( 'Must be a an array', 'newslist' )
				);
				...
				<div class="error">
					<?php foreach ( self::$errors as $key => $value ) : ?>
						<p>
							...
							echo sprintf(
								'<strong>%s</strong> %s',
								esc_html__( 'Newslist Customizer:', 'newslist' ),
								esc_html( $value )
							);
							?>
						</p>
...
<?php
		...
		private static function get_panel_arg( $panel ) {
			$args = array(
				'title' => empty( $panel['title'] ) ?
					esc_html__( 'No Title Specified.', 'newslist' ) :
					$panel['title'],
			);
		...
		private static function get_control_arg( $section, $field, $prefix_on_section = true ) {
			...
			if ( isset( $field['active_callback'] ) ) {
				$acb = $field['active_callback'];
				if ( is_array( $acb ) ) {
					$args['active_callback'] = $acb;
				} else {
					$args['active_callback'] = self::fn_prefix( $acb );
				}
			}
		...
		private static function get_section_arg( $panel_id = false, $section ) {
			$args = array(
				'title' => empty($section['title']) ? esc_html__('No Title Specified.', 'newslist') : $section['title'],
			);
		...
		public static function register( $wp_customize ) {
			...
			foreach ( self::$settings as $id => $args ) {
				...
				$wp_customize->add_setting(
					$id,
					array_merge( $args, array( 'sanitize_callback', $args['sanitize_callback'] ) )
				);
...
add_action( 'customize_register', array( Newslist_Customizer::get_instance(), 'register' ) );

if ( !function_exists( 'newslist_get' ) ) :
	/**
	 ...
	 * @package Newslist WordPress Theme
	 */
	function newslist_get( $id ) {
		$id = Newslist_Helper::with_prefix( $id );
		return Newslist_Customizer::get( $id );
	}
