 ...
 */
if ( !class_exists( 'Newslist_Helper' ) ) :
	class Newslist_Helper {
...
?>
			<header id="masthead" <?php self::schema_body( 'header' ); ?> class="<?php echo esc_attr( self::with_prefix( 'site-header' ) ) ?>">
			<?php
		}
		...
		?>
			<a class="skip-link screen-reader-text" href="#content">
				<?php esc_html_e( 'Skip to content', 'newslist' ); ?>
			</a>
			<?php
		}
		...
		public static function the_header_image() {
			...
			} elseif ( is_archive() || has_header_image() ) {
		...
		public static function excerpt_more( $text = false ) {
			...
			if ( '' != newslist_get( 'read-more-text' ) ) {
				$more = sprintf(
					'<a href="%1$s">%2$s<i class="fa fa-long-arrow-right" aria-hidden="true"></i></a>',
					esc_url( get_the_permalink() ),
					esc_html( newslist_get( 'read-more-text') )
				);
			} else {
				$more = '';
			}
		...
		public static function get_comment_number() {
			if ( !post_password_required() && ( get_comments_number() ) ) {
				echo '<span class="newslist-comments">';
				comments_popup_link(
					'<i class="fa fa-comment"></i> ' . esc_html__( 'Leave a comment', 'newslist' ),
					'<i class="fa fa-comment"></i> ' . esc_html__( '1 response', 'newslist' ),
					'<i class="fa fa-comments"></i> % ' . esc_html__( 'responses', 'newslist' )
				);
				echo '</span>';
			}
		}
		...
		public static function get_assets_version()	{
			return newslist_get( 'assets-version' );
		}
		...
		public static function get_menu( $location = 'primary', $echo = false ) {
			...
			switch ( $location ) {
				...
				case 'social-menu-header' :
					$menu = wp_nav_menu(array(
						'menu_id' 			=> 'social-menu-header',
						'theme_location' => $location,
						'fallback_cb' 		=> false,
						'echo' 					=> $echo,
						'menu_class'		=> 'newslist-social-menu',
						'link_before'	    => '<span>',
						'link_after'	     => '</span>',
						'depth'				   => 1
					) );
		...
		public static function posts_navigation( $echo = true )	{
			...
			if ( !$infinity_module_active || !Newslist_Helper::is_active_plugin( 'jetpack' ) ) {
				the_posts_pagination(
					array(
						'mid_size'  => 2,
						'prev_text' => esc_html__( 'Previous', 'newslist' ),
						'next_text' => esc_html__( 'Next', 'newslist' ),
					)
				);
			}
		}
		...
		public static function post_content_navigation() {
			wp_link_pages(array(
				'before'      => '<div class="page-links">' . esc_html__( 'Pages:', 'newslist' ),
				'after'       => '</div>',
		...
		public static function single_post_navigation()	{
			the_post_navigation(array(
				'prev_text' => '<span class="screen-reader-text">' . esc_html__( 'Previous Post', 'newslist' ) . '</span><span class="nav-title">%title</span>',
				'next_text' => '<span class="screen-reader-text">' . esc_html__( 'Next Post', 'newslist' ) . '</span><span class="nav-title">%title</span>',
			));
		}
		...
		public static function the_date( $post_id = null, $status = 'posted' ) {
            ...
            printf(
				...
				( 'posted' == $status ) ? esc_html__( 'On', 'newslist-pro' ) : esc_html__( 'Updated on', 'newslist-pro' ),
                $time_tag
            );
            endif;
        }
		...
		public static function the_breadcrumb() {
			...
			?>
				<div id="<?php echo esc_attr( self::with_prefix( 'breadcrumb' ) ) ?>" class="<?php echo esc_attr( $classes); ?>">
					<?php newslist_breadcrumb_trail( $breadcrumb_args ); ?>
				</div><!-- #breadcrumb -->
			<?php return;
			endif;
		}
			...
			<div class="<?php echo esc_attr( self::with_prefix( 'inner-banner' ) ) ?>">
				<header class="entry-header">
					...
					<?php } elseif ( is_home() ) {
						...
						$blog_title = apply_filters( self::fn_prefix( 'blog_title' ), true ); ?>
						<h2 class="entry-title"><?php echo esc_html( $blog_title ) ?></h2>
					...
					?>
						<h2 class="entry-title">
							<?php
								printf(
									esc_html__( 'Search Results for: %s', 'newslist' ),
		...
		/**
		 * Font family used in Newslist Theme
		 *
		 * @static
		 * @access public
		 * @return object
		...
		public static function get_font_family() {
			return apply_filters( self::fn_prefix( 'standard_fonts_array' ), array(
				'font-1'  => esc_html__( 'Lato', 'newslist' ),
				'font-2'  => esc_html__( 'Oswald', 'newslist' ),
				'font-3'  => esc_html__( 'Montserrat', 'newslist' ),
				'font-4'  => esc_html__( 'Roboto', 'newslist' ),
				'font-5'  => esc_html__( 'Raleway', 'newslist' ),
				'font-6'  => esc_html__( 'Playfair Display', 'newslist' ),
				'font-7'  => esc_html__( 'Fjalla One', 'newslist' ),
				'font-8'  => esc_html__( 'Alegreya Sans', 'newslist' ),
				'font-9'  => esc_html__( 'PT Sans Narrow', 'newslist' ),
				'font-10' => esc_html__( 'Open Sans', 'newslist' ),
				'font-11' => esc_html__( 'Poppins', 'newslist' ),
			));
		}
		...
		public static function get_google_font() {
			...
			$google_fonts[] = self::uglify( self::get_font( newslist_get( 'body-font' ) ), $condition );
			$google_fonts[] = self::uglify( self::get_font( newslist_get( 'heading-font' )), $condition );
			$google_fonts[] = self::uglify( self::get_font( newslist_get( 'site-info-font' )) , $condition );
